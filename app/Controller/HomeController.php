...
class HomeController extends AppController {
	...
	function beforeFilter(){
		parent::beforeFilter();
		$this->Auth->allow(array('index','details','gp_consultation','health_information','services','contact','about','care_quality_commission','our_team','timetable','find_conditions','condition','callback','pharmistic_advice','learn'));		
	}	
	...
	function health_information(){
		...
		$PageVar['slug'] = 'Health Information';
	...
	function admin_index() {
		...
		$PageVar['user_count'] = $this->Core->findCount('User',array('User.user_status'=>'Active'));
		$PageVar['practitioner_count'] = $this->Core->findCount('User',array('User.user_status'=>'Active','User.user_role_id'=>2));
		$PageVar['template_count'] = $this->Core->findCount('EmailTemplate',array('EmailTemplate.template_status'=>'Active'));
		$PageVar['services_count'] = $this->Core->findCount('Service',array('Service.status'=>'Active'));
		$PageVar['condition_count'] = $this->Core->findCount('Condition',array('Condition.status'=>'Publish'));
		$this->set('PageVar',$PageVar);
	}
